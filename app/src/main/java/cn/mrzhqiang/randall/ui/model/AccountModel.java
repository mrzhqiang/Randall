...
public final class AccountModel {
  ...
  private static final String QUERY_USERNAME =
      "SELECT * FROM " + RandallAccount.NAME + " WHERE " + RandallAccount.COL_USERNAME + " = ?";
  ...
  public void addAccount(@NonNull String username, @NonNull String password,
    ...
    Observable.just(RandallAccount.create(username, password, RandallAccount.Status.DEFAULT))
        .subscribeOn(Schedulers.io())
        .map(new Func1<RandallAccount, Long>() {
          @Override public Long call(RandallAccount account) {
            if (checkExists(account.username())) {
              throw new RuntimeException("账号已经存在");
            }

            return db.insert(RandallAccount.NAME, new RandallAccount.Builder(account).build());
          }
        })
    ...
    Observable.just(String.valueOf(id))
        ...
        .map(new Func1<String, Integer>() {
          @Override public Integer call(String id) {
            return db.delete(RandallAccount.NAME, RandallAccount.COL_ID + "=?", id);
          }
        })
        .observeOn(AndroidSchedulers.mainThread())
        .subscribe(subscriber);
  ...
  public void changePassword(final long id, @NonNull String password,
    ...
    Observable.just(password).subscribeOn(Schedulers.io()).map(new Func1<String, Integer>() {
      @Override public Integer call(String password) {
        return db.update(RandallAccount.NAME, new RandallAccount.Builder().password(password).build(),
            RandallAccount.COL_ID + "=?", String.valueOf(id));
      }
    }).observeOn(AndroidSchedulers.mainThread()).subscribe(subscriber);
  }

  /** 更新账户状态 */
  public void updateStatus(final long id, RandallAccount.Status status,
      @NonNull Subscriber<Integer> subscriber) {
    Observable.just(status).subscribeOn(Schedulers.io()).map(new Func1<RandallAccount.Status, Integer>() {
      @Override public Integer call(RandallAccount.Status status) {
        return db.update(RandallAccount.NAME, new RandallAccount.Builder().status(status).build(),
            RandallAccount.COL_ID + "=?", String.valueOf(id));
      }
    }).observeOn(AndroidSchedulers.mainThread()).subscribe(subscriber);
  }
  ...
  public void queryList(Subscriber<List<RandallAccount>> subscriber) {
    String delete = String.valueOf(RandallAccount.Status.DELETE.ordinal());
    subscription.add(db.createQuery(RandallAccount.NAME, QUERY_LIST, delete)
        .mapToList(RandallAccount.MAPPER)
        .observeOn(AndroidSchedulers.mainThread())
        .subscribe(subscriber));
  }
